...
class AuthorizationClient implements Serializable {
  ...
  class WebViewAuthHandler extends AuthHandler {
    ...
    boolean tryAuthorize(final AuthorizationRequest request) {
      ...
      WebDialog.Builder builder = new AuthDialogBuilder(getStartActivityDelegate().getActivityContext(), applicationId, parameters).setOnCompleteListener(listener);
  ...
  class KatanaLoginDialogAuthHandler extends KatanaAuthHandler {
    ...
    boolean tryAuthorize(AuthorizationRequest request) {
      Intent intent = NativeProtocol.createLoginDialog20121101Intent(context, request.getApplicationId(), new ArrayList<String>(request.getPermissions()), request.getDefaultAudience().getNativeProtocolAudience());
  ...
  class KatanaProxyAuthHandler extends KatanaAuthHandler {
    ...
    @Override
    boolean tryAuthorize(AuthorizationRequest request) {
      Intent intent = NativeProtocol.createProxyAuthIntent(context, request.getApplicationId(), request.getPermissions());
