...
: blockSituations(blockSituations), isLocked(isLocked), comboSituations(comboSituations), state(state), stateTick(stateTick), garbageSituations(garbageSituations), garbageStackSituation(garbageStackSituation),
skillChainLevel(skillChainLevel), cursorPosition(cursorPosition), scrollingOffset(scrollingOffset), freezingTime(freezingTime), gameOver(gameOver), wallOffset(wallOffset), gracing(gracing),
score(score), clearing(clearing) {
}
