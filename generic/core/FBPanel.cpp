...
Combo *FBPanel::getComboByBlock(Block *block) const {
  for (set<Combo *>::const_iterator combo = combos.begin(); combo != combos.end(); combo++) {
...
FBPanel::Clearing *FBPanel::getClearingByBlock(Block *block) const {
  for (set<Clearing *>::const_iterator clearing = clearings.begin(); clearing != clearings.end(); clearing++) {
